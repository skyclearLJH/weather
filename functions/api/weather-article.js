...
- 바로 앞 문장과 동일한 지역 묶음을 이어서 설명할 때는 지명을 반복하지 않습니다. 여러 지역은 "이들 지역에서는", 한 지역은 "이 지역에서는"으로 받습니다. 앞 문장과 지역 범위가 다르거나 가리키는 대상이 모호하면 대명사를 쓰지 않습니다.
...
- 입력에 최근 30분 강도 경향이 있으면 사실에 적힌 대로 "전반적으로 강해지는 흐름", "전반적으로 약해지는 흐름", "강해졌다 약해지기를 반복", "비슷한 강도를 유지" 가운데 해당 표현을 자연스럽게 연결합니다. 단일 시점끼리 다시 비교하거나 이를 비구름의 이동·새로운 발달·강수 구역 확대나 축소로 해석하지 않습니다.
- 원고에는 "레이더상"이라는 표현을 쓰지 않습니다. 강도 경향은 "최근 30분 동안 비의 강도가 전반적으로 강해지는 흐름입니다"처럼 바로 설명합니다.
- 원고에는 "같은 위치에서", "같은 위치의"라는 표현을 어떤 경우에도 쓰지 않습니다.
...
- 초단기예측에 초반 시각과 한 시간 뒤 사실이 함께 있으면 두 시점 모두 반영합니다. 초반의 강한 지역만 요약하고 한 시간 뒤의 지역을 빼면 안 됩니다.
- 한 시간 뒤 새롭게 주요 지역으로 제시된 곳은 현재 지상 관측이 없더라도 예측 문단에 포함합니다. 같은 지역이 두 시점에 모두 강하면 "한 시간 뒤에도", 초반보다 약해지면 묶어서 "비의 강도가 다소 약해질"로 설명합니다.
- 앞 문장에서 한 지역의 비가 약해진다고 설명하고 다음 문장에서 다른 지역의 강한 비를 대비할 때는 "한편"이 아니라 "반면"으로 연결합니다.
- 한 시간 뒤 가장 강한 지역이 별도 사실로 제시되면 "특히"를 활용해 분명히 강조하되, 시점별 차이를 이동 방향이나 강수 구역의 확대·축소로 해석하지 않습니다.
- 강도 변화 전망에는 "예측 후반으로 갈수록"이라는 표현을 쓰지 않고, "비의 강도는 점차 약해질 것으로 예상됩니다"처럼 바로 설명합니다.
...
- 최신 레이더의 집중 지역 현황 → 가장 주목할 강수 핵 → 최근 강도 변화 → 이를 뒷받침하는 묶은 지상 관측 → 상황의 의미와 주의점 → 초단기예측의 초반과 한 시간 뒤 변화 순으로 자연스럽게 이어갑니다.
- "특히", "최근 30분 동안", "실제 지상 관측에서도", "이처럼", "반면" 같은 연결 표현을 사실관계에 맞게 활용해 문단 사이의 흐름을 만듭니다.
...
- 묶인 지상 관측은 1~2문장으로 끝내고, 시점별 차이가 있는 초단기예측은 2~3문장 안에서 초반·약화 지역·한 시간 뒤 강한 지역을 간명하게 설명합니다.
