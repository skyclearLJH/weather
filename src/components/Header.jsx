...
const Header = ({ selectedRegion, onChangeRegion, onRefresh }) => {
  return (
      ...
      <div className="max-w-screen-xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          ...
          <div className="flex items-center gap-2 text-blue-100 text-xs sm:text-sm font-medium">
            <button onClick={onRefresh} className="p-1 hover:bg-white/10 rounded-full transition-colors" title="새로고침">
              <RefreshCcw size={16} />
            </button>
            <span className="hidden sm:inline">정시 업데이트됨</span>
          </div>
        </div>
      </div>
    </header>
